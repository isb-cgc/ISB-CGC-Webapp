...
def extended_logout_view(request):
    ...
    try:
        directory_service.members().delete(groupKey=CONTROLLED_ACL_GOOGLE_GROUP,
                                           memberKey=str(user_email)).execute(http=http_auth)
    ...
    try:
        ...
        directory_service.members().insert(groupKey=OPEN_ACL_GOOGLE_GROUP,
                                           body=body).execute(http=http_auth)
...
def unlink_accounts(request):
    ...
    try:
        ...
        logger.info("{} could not be deleted from %s, probably because they were not a member: {}"
...
def nih_login(request):
    ...
    if guid:
        ...
        if contents:
            ...
            user_email = User.objects.get(id=request.user.id).email
            try:
                result = directory_service.members().get(groupKey=CONTROLLED_ACL_GOOGLE_GROUP,
                                                         memberKey=user_email).execute(http=http_auth)
            ...
            # if the member doesn't yet exist in the google group...
                ...
                if is_dbGaP_authorized:
                    ...
                    directory_service.members().insert(
                        groupKey=CONTROLLED_ACL_GOOGLE_GROUP,
                        body=body
                    ).execute(http=http_auth)
                    logger.info("User {} added to {}.".format(user_email, CONTROLLED_ACL_GOOGLE_GROUP))
