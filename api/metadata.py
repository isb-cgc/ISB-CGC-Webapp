...
def query_samples_and_studies(parameter, bucket_by=None):
    ...
    try:
        cursor = db.cursor(MySQLdb.cursors.DictCursor)
        start = time.time()
        cursor.execute('SELECT sample_id, study_id FROM cohorts_samples WHERE cohort_id=%s;', (parameter,))
        stop = time.time()
        log_str = "[BENCHMARKING] Time to query sample IDs for cohort '" + parameter + "': " + (stop-start).__str__()
...
def count_metadata(user, sample_ids=None, filters={}, cohort_id=None):
    counts_and_total = {}
    valid_attrs = {}
    sample_tables = {}
    table_key_map = {}
    ...
    for key in sample_ids:
        sample_list = sample_ids[key]
        sample_ids[key] = {
            'SampleBarcode': build_where_clause({'SampleBarcode': sample_list}),
            'sample_barcode': build_where_clause({'sample_barcode': sample_list}),
        }
    ...
    try:
        ...
        for key, feature in valid_attrs.items():
            ...
            counts_and_total['counts'].append(MetadataAttributeValues(name=feature['name'], values=value_list, id=key, total=feature['total']))
...
class Meta_Endpoints_API_v2(remote.Service):
    ...
    def metadata_counts_platform_list(self, request):
        ...
        if request.__getattribute__('cohort_id') is not None:
            ...
            sample_ids = query_samples_and_studies(cohort_id, )
        ...
        counts_and_total = count_metadata(user, samples_by_study, filters, cohort_id)
        ...
        try:
            ...
            cursor.execute(query_str, value_tuple)
            stop = time.time()
            logger.debug("[BENCHMARKING] Time to query platforms in metadata_platform_list for cohort '" + str(
