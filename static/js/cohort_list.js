...
require([
    ...
    var toggle_buttons = function () {
        if ($('tr:not(:first) input[type="checkbox"]:checked').length == 0) {
            ...
            $('.page-action-group .btn.com-coh').attr('title', 'Select any two cohorts.');
        } else {
            if ($('tr:not(:first) input[type="checkbox"]:checked').length >= 2) {
                ...
                if ($('tr:not(:first) input[type="checkbox"]:checked').length > 2) {
                    //$('.page-action-group .btn:not(.com-coh)').removeAttr('title');
                    $('.page-action-group .btn.com-coh').prop('disabled', 'disabled');
                    $('.page-action-group .btn.com-coh').attr('title', 'Select any two cohorts.');
                }
            } else {
                $('.page-action-group .btn:not(.set-ops,.com-coh)').removeAttr('title');
                $('.page-action-group .btn:not(.owner-only,.set-ops,.com-coh)').removeAttr('disabled');
            ...
            canDelOrShare && $('.owner-only').removeAttr('disabled') && $('.page-action-group .btn:not(.set-ops,.com-coh)').removeAttr('title');
    ...
    var checkbox_change_callback = function () {
        ...
        if ($(this).is(':checked') && $(this).val() !== 'on') {
            $('#cohort-apply-to-workbook').append($('<input>', {
                type: 'hidden',
                name: 'cohorts',
                value: $(this).val()
            }));
    ...
    $(".com-coh").on("click", function () {

        var sel_cohort_ids = [];
        $('#saved-cohorts-list input[type="checkbox"], #public-cohorts-list input[type="checkbox"]').each(function () {
            if ($(this).is(':checked') && $(this).val() != 'on') {
                sel_cohort_ids.push($(this).val());
            }
        });

        if (sel_cohort_ids.length < 2 || sel_cohort_ids.length > 2) {
        ...
        if (sel_cohort_ids.length > 0) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: BASE_URL + '/compare/validate_cohorts',
                data: JSON.stringify({sel_cohort_ids: sel_cohort_ids}),
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("X-CSRFToken", csrftoken);
                },
                success: function (data) {
