...
define([
    ...
    function generate_plot(plot_element, x_attr, y_attr, color_by, cohorts, cohort_override) {
        ...
        plot_element.find('.plot-div').empty();
        plot_element.find('.legend').empty();
        plot_element.find('.pairwise-result').empty();
    ...
    function saveViz() {
        ...
        $('.plot').each(function (index) {
            ...
            var cohort_ids = $.map($(this).find('.cohort-listing div'), function (d) {
                return $(d).attr('value');
            });
    ...
    //this used to be from $('form[action="/cohorts/save_cohort_from_plot/"]')
    function saveCohortFromPlot(){
        $.ajax({
            type: 'POST',
            url: base_url + '/cohorts/save_cohort_from_plot/',
            data: $(this).serialize(),
    ...
    return {
        ...
        save_cohort_from_plot : saveCohortFromPlot
    };
});
/**
