...
if [ -z "static_last_commit.txt" ] || [ ${STATIC_LAST_COMMIT} != $(cat "static_last_commit.txt") ]; then
    echo "Beginning rsync of /static..."
    ./google-cloud-sdk/bin/gsutil rsync -R static/ gs://webapp-dev-static-files/static
    git rev-list -1 HEAD -- "static" > static_last_commit.txt
    ./google-cloud-sdk/bin/gsutil cp "static_last_commit.txt" gs://${GCLOUD_BUCKET_DEV}/
else
