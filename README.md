...
Google App Engine using Python 2.7, Django 1.7.1, MySQL 5.6

This app is set up to run and deploy on various Google Cloud Project. It uses a zip version of Django instead of the natively supported version.

Required libraries not included (this should be zipped and placed in a /lib/ directory in the root of this project:
- allauth
- apiclient
- django==1.7.1
- googleapiclient
...
`gcloud auth login`
4. You will need access to the ISB-CGC Google Drive directory (please ask project members for this information) to download the following:
...
8. Create databases dev, stage, test, and prod. All operations on databases should be done for all the databases you plan to use.
9. `pip install pycrypto` - Needed as previously noted
10. In GenespotRE/dev_settings.py, stage_settings.py, test_settings.py, and prod_settings.py, change the local database settings to match your local settings. You many need to change the name of the user, and password to match your local settings. Do not commit these changes.  Do similarly for scripts/add_site_ids.py and scripts/add_alldata_cohort.py .
11. In all settings files, make sure to point them to the correct private key .pem and .json files.
12. In the settings files, be sure to change the DEVLOPER_COHORT_TABLE_ID to a unique table dev_deployment_cohorts instance in Big Query.
13. run `python manage.py makemigrations` - Setting up Django models
14. run `python manage.py migrate` - Setting up Django models
15. run `python manage.py createsuperuser` and leave the superuser's email blank.
16. enter a mysql shell and run `CREATE USER 'django'@'localhost' IDENTIFIED BY 'password'`
    and `GRANT SELECT, INSERT, UPDATE, DELETE ON <DATABASE NAME>.* TO 'django'@'localhost'`
17. run `python scripts/add_site_ids.py`, making sure the settings in the script are accurate.
18. Import metadata_samples and metadata_attr tables with `mysql -u root -p <databasename> < dev_2015-07-09_metadata_attr_samples.sql` .
19. Import the feature definition tables by using a .sql dump file. There are multiple feature definition tables that are required.
  - feature_defs_cnvr
  - feature_defs_gexp
  - feature_defs_gnab
  - feature_defs_meth
  - feature_defs_mirn
  - feature_defs_rppa
20. run `dev_appserver.py .`
21. Go to [http://localhost:8080/](http://localhost:8080) and hope for the best!
22. If the site works, go to [http://localhost:8080/admin](http://localhost:8080/admin) and enter the superuser name and password you created.
...
When running the app through dev_appserver.py, it is simulating the environment on the cloud. It is recommended to run the app this way, and not through the django manage.py runserver.
