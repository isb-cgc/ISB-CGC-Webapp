...
class LoggingMiddleware(object):
    ...
    def _log_request(self, request, response):
        method_path = "Request: {} {}".format(request.method, request.get_full_path())
    ...
    def process_response(self, request, response):
        resp_log = "Response: {} {} - {}".format(request.method, request.get_full_path(), response.status_code)
        skip_logging, because = self._should_log_route(request)
        if skip_logging:
            if because is not None:
    ...
    def _log_resp(self, level, response, logging_context):
        self.logger.log(level, "Response Content-Type is {}".format(response.get("Content-Type", "unknown")), logging_context)
        if IS_DJANGO_VERSION_GTE_3_2_0:
            response_headers = response.headers
        else:
            response_headers = response._headers
        self.logger.log(level, response_headers, logging_context)
        if re.match("^(application/json|text/csv)", response.get("Content-Type", ""), re.I):
