...
ONCOPRINT_VIEW_DEBUG_MODE = False
...
def get_table_row_id(tumor_type):
    return "oncoprint_row_{0}".format(tumor_type)


def build_oncoprint_regions(protein_data):
    return [{
        'type': 'exon',
        'start': 0,
...
class OncoPrintViewDataBuilder(object):
    def build_view_data(self, hugo_symbols, oncoprint_cohort_info, cohort_id_list, removed_row_statistics, tables_used):
        context = get_genes_tumors_lists()

        cohort_info_map = {str(item['id']): item for item in oncoprint_cohort_info}

        plot_data = {
            'gene_labels': hugo_symbols,
        }
        ...
        plot_data['regions'] = build_oncoprint_regions(plot_data['protein'])
