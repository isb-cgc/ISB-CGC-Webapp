...
END
  AS has_related
FROM
  `idc-dev-etl.{data_version}_dev.dicom_derived_all`
GROUP BY
  PatientID,
  StudyInstanceUID,
  crdc_study_uuid
), study_sizes AS (
  SELECT crdc_study_uuid,
  SUM(instance_size) AS study_size
  FROM `idc-dev-etl.{data_version}_pub.dicom_all`
  GROUP BY crdc_study_uuid
)
SELECT study_index.*, study_sizes.study_size AS instance_size
FROM study_index
LEFT JOIN study_sizes
ON study_sizes.crdc_study_uuid = study_index.crdc_study_uuid
;